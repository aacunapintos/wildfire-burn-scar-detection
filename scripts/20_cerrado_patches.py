"""
Generate overlapping inference patches (ZS mode) for the Cerrado wildfires
(Alto Araguaia, Mato Grosso Brazil — 2023). Tropical dry savanna.

Spectrally the closest biome to Corrientes — highest expected IoU.
STRIDE=128 (50% overlap). Output directly to Google Drive ZS folder.

Run from the project root (after 26_cerrado_download.py):
    python -u scripts/27_cerrado_patches.py
"""
...
SITE           = "cerrado"
CRS_UTM        = CRS.from_epsg(32722)   # UTM 22S — Mato Grosso
DNBR_THRESHOLD = 0.12                   # Savanna: lower threshold (similar to Corrientes)
PATCH_SIZE     = 256
STRIDE         = 128
MAX_CLOUD_FRAC = 0.20                   # Dry season — strict filter
...
with open(MANIFEST, "w") as f:
    json.dump({"site": SITE, "stride": STRIDE, "patch_size": PATCH_SIZE, "crs_epsg": 32722,
               "dnbr_thresh": DNBR_THRESHOLD, "total": total_patches, "tiles": manifest}, f, indent=2)

print(f"\n{'='*65}")
print(f"Done. Total patches: {total_patches}")
print("All ZS sites complete. Check G:/Mon Drive/GeoAI/wildfire-spread/data/zs/")
