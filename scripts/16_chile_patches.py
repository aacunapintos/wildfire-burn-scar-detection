"""
Generate overlapping inference patches (ZS mode) for the Valparaiso wildfire
(Chile — February 2023). Mediterranean WUI biome.

STRIDE=128 (50% overlap). Output directly to Google Drive ZS folder.

Run from the project root (after 22_chile_download.py):
    python -u scripts/23_chile_patches.py
"""
...
SITE           = "chile"
CRS_UTM        = CRS.from_epsg(32719)   # UTM 19S
DNBR_THRESHOLD = 0.15
PATCH_SIZE     = 256
STRIDE         = 128
MAX_CLOUD_FRAC = 0.25
...
with open(MANIFEST, "w") as f:
    json.dump({"site": SITE, "stride": STRIDE, "patch_size": PATCH_SIZE, "crs_epsg": 32719,
               "dnbr_thresh": DNBR_THRESHOLD, "total": total_patches, "tiles": manifest}, f, indent=2)

print(f"\n{'='*65}")
print(f"Done. Total patches: {total_patches}")
print("Next: python -u scripts/17_california_download.py")
